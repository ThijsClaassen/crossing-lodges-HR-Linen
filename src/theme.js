...
// bar matches theirs: logo + company switcher + tabs top-to-bottom on the
// left, footer with role badge/refresh/sign-out at the bottom. This app has
// no global location scoping (that's per-tab, e.g. Employees/Schedule pick
// their own lodge internally), so unlike Food/Beverage/Curio there's no
// sidebar loc-switcher or mobile-loc-bar here. Class names deliberately
// match Ops's theme.js 1:1 so a future layout tweak can be copied across
// every app without translation. Display toggling between the sidebar and
// the mobile bottom-sheet nav is done purely via these CSS classes (no
// inline `display` style on the elements themselves) — an inline style
// would always win over a non-`!important` media-query rule, which is
// exactly the trap the app's old `.desktop-tab-row`/`.mobile-nav-bar`
// inline-styled version fell into.
export const css = `
  ...
  /* Topbar — always visible (both desktop and mobile), same as Ops: page
     ...
     sign out) that live redundantly here as well as in the desktop
     sidebar, so they're still reachable once the sidebar hides on mobile. */
